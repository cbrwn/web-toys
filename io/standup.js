    ...
    generateStats: function () {
      let createAward = (award, emojiStats) => {
        ...
        return {
          ...
          title: award.title,
          desc: `${
            award.type == "receiver" ? "received" : "sent"
          } ${highestValue}x ${award.emoji}`,
        };
      };
  ...
  setup: function (io) {
    ...
    ns.on("connection", (socket) => {
      ...
      socket.on("setName", (data, callback) => {
        ...
        if (
          socket.client.room != null &&
          rooms.hasOwnProperty(socket.client.room)
        ) {
          rooms[socket.client.room].sendRoomState(
            socket.to(socket.client.room)
          );
        }
      ...
      socket.on("cancel", (data, callback) => {
        ...
        if (rooms[socket.client.room].state !== "waiting") {
          return callback({
            status: false,
            message: "room isn't in progress!",
          });
        }
      ...
      socket.on("react", (data, callback) => {
        ...
        if (
          socket.client.lastReactTime != null &&
          nowTime - socket.client.lastReactTime < 1000
        ) {
        ...
        socket
          ...
          .emit("react", { emoji: data.emoji, who: socket.client.id });
        socket.client.lastReactTime = nowTime;

        let room = rooms[socket.client.room];
        room.onEmojiSend(
          socket.client.id,
          room.people[room.order[0]].id,
          data.emoji
        );
