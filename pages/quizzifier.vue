    ...
    <div class="flex flex-col w-full justify-center py-8 gap-y-3">
      ...
      <ContentContainer class="overflow-clip">
        <!-- 
				...
				-->
        <div
          class="absolute h-full w-full top-0 bg-gray-200 dark:bg-slate-600 z-50 duration-300"
          :class="{
            ['-translate-x-full transition-none']:
              quizState == goalQuizState && transitionState == 1,
            ['translate-x-0 ease-in transition-transform']:
              quizState != goalQuizState,
            ['translate-x-full ease-out']:
              quizState == goalQuizState && transitionState == 0,
          }"
        ...
        <div v-if="quizState == 'idle'" class="flex flex-col items-center">
          ...
          <div class="flex flex-row gap-4 justify-center">
            <QuizSelector
              ...
              :selected="quizSettings.source == key"
          ...
          <div class="flex flex-row gap-4 justify-center">
            <QuizSelector
              ...
              :selected="quizSettings.type == key"
          ...
          <div
            class="flex items-center justify-center text-3xl h-16 w-48 mt-12 bg-green-500 rounded-2xl cursor-pointer"
        ...
        <!--
				...
				-->
        <div
          v-else-if="quizState == 'finished' || quizState == 'quit'"
        ...
        >
          ...
          <div class="flex flex-col mb-8">
            <span
              class="text-9xl font-extrabold font-sans"
              v-html="gradeHtml"
            />
          </div>
          ...
          <div class="flex flex-col">
            ...
            <div class="flex flex-row">
              ...
              />
              <QuizScoreElement
                :score="quizStats.incorrect"
                type="incorrect"
                color="bg-red-600"
              />
              <QuizScoreElement
                v-if="quizType.questionsRemaining()"
                :score="quizType.questionsRemaining()"
                type="unanswered"
                color="bg-gray-500"
              />
            </div>
          ...
          <div
            class="flex items-center justify-center text-3xl h-16 w-48 mt-8 bg-green-500 rounded-2xl cursor-pointer"
        ...
        <div class="w-full" v-else>
          ...
          <div class="flex items-start mb-4">
            <!-- quit/cheat buttons -->
            <div
              class="flex justify-center items-center rounded-lg bg-red-700 w-16 h-8 cursor-pointer text-white"
            ...
            <div class="flex flex-grow flex-col items-center justify-center">
              <div class="text-4xl">
                <span class="dark:text-green-400 text-green-500">{{
                  quizStats.correct
                }}</span>
                :
                <span class="dark:text-red-400 text-red-500">{{
                  quizStats.incorrect
                }}</span>
              </div>
              ...
              <!-- progress bar -->
              <div
                v-if="quizType.progressText() != null"
                class="w-1/2 h-8 mt-2"
              >
                <div
                  class="w-full h-full bg-gray-300 dark:bg-slate-500 rounded-3xl overflow-clip"
                >
                  <div
                    class="flex justify-end items-center h-full bg-blue-400 transition-all duration-300"
                    :style="`width: ${quizType.progressValue()}%`"
                  >
                    <p
                      class="mr-2 text-white transition-all"
                      :class="{
                        ['mr-2']: quizType.progressValue() > 5,
                        ['-mr-6 text-black dark:text-white']:
                          quizType.progressValue() <= 5,
                      }"
                    >
            ...
            <div
              class="flex justify-center items-center rounded-lg bg-violet-700 w-16 h-8 cursor-pointer"
              v-on:click="cheat_almostFinished"
              v-if="cheatingEnabled"
            >
              cheat
            </div>
          ...
          <!--
					...
					-->
          <div
            class="flex items-center flex-col gap-5 w-full overflow-clip"
            v-if="remainingAnswers != null"
          >
            <!-- Image(s) (including placeholder used for animation) -->
            <div class="flex items-center flex-col w-full">
              <div :style="currentImageStyle">
                <img
                  :src="currentAnswer.imagePath"
                  class="h-48 sm:h-96 pointer-events-none"
                />
              </div>
            ...
            <div
              class="text-4xl mt-2 mb-3"
              v-if="guessState != 'idle'"
              :class="{
                ['text-green-500']: guessState == 'correct',
            ...
            >
              <span
                class="inline-block"
                :class="{
                  ['animate-spin']: guessState == 'correct',
                  ['animate-ping']: guessState == 'incorrect',
                }"
                >{{ answerEmoji }}</span
              >
...
export default {
  ...
  computed: {
    ...
    cheat_almostFinished() {
      while (this.quizType.questionsRemaining() > 1) {
        ...
        if (Math.random() < 0.01) this.quizStats.incorrect++;
        else this.quizStats.correct++;
      }
    },
    quizSource() {
    ...
    correctPercentage() {
      ...
      return Math.round(
        (this.quizStats.correct / this.totalAnswersCount) * 100
      );
    },
    letterGrade() {
    ...
    imageTransition() {
      ...
      if (this.guessState == "idle") transitionDuration = "0.5s";
  ...
  methods: {
    ...
    selectNewAnswer() {
      ...
      this.currentAnswer.imagePath =
        newAnswer.images[Math.floor(Math.random() * newAnswer.images.length)];
    ...
    isCorrectAnswer(guess) {
      ...
      let sanitisedCorrectAnswers =
        this.currentAnswer.correctAnswers.map(sanitise);
    ...
    guessPress(event) {
      ...
      if (event.key == "Enter") {
        ...
        if (isCorrect) this.quizStats.correct++;
        else this.quizStats.incorrect++;
