            ...
            <div class="flex-grow">
              ...
              <div class="flex flex-col flex-grow pt-4" v-if="roomState.state == 'waiting'">
                ...
                <p class="mt-5 opacity-50">
                  pro tip: bookmark this url to come back to this room every
                  morning!
                </p>
              ...
              <div class="flex flex-col items-center h-full" v-else-if="roomState.state == 'running'">
                ...
                <div class="flex flex-col items-center mt-5">
                  ...
                  <div v-else-if="!isUpNext && !isMyTurn && !hasHadTurn">
                    <p class="opacity-60">we'll come back to you at the end!</p>
                  </div>
...
export default {
  ...
  methods: {
    ...
    joinRoom(roomId, claimHost, onFail) {
      ...
      this.socket.emit(
        "join",
        { roomId: roomId, claimHost: claimHost, name: playerName },
        (res) => {
          if (!res.status) {
            if (onFail) {
              onFail(roomId);
